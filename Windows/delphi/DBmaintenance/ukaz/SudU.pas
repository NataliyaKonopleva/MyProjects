...
type
  TFmPer = class(TForm)
    ...
    procedure Dopl1(Sender: TObject);
...
{$R *.dfm}
procedure TFmPer.Dopl1(Sender: TObject);
var d:TDateTime;
    delo:integer;
begin
 d:=StrToDate('01.01.1995');
 Tprn.Filter:='pnsapp<=';
 Tprn.Filtered:=true;
 ...
 while not Tprn.Eof do
  begin
   delo:=Tprn['delo'];
   Tprn.Edit;
   if Tpp.Locate('delo',delo,[])
   then
    begin
     Tprn.FieldValues['salpst1']:=Tpp['doplo'];
     Tprn.FieldValues['salpst0']:=(Tprn['salpst']/100*25+Tprn['increg']/100*25+Tprn['incwsl']/100*25+Tprn['sumcof']/100*25)/100*Tprn['penprc']*38;
     Tprn.FieldValues['dopl']:=Tprn['salpst0']-Tprn['salpst1'];
    end;
   Tprn.Next;
  end;
end;
...
begin
 ...
 Tpp.Open;
 try
 if not FileExists('F:\DATA\ukaz.MDX') then
  begin
   Tprn.AddIndex('ukaz2','nampns',[ixExpression]);
   Tprn.AddIndex('ukaz1','delo',[]);
  end;
 try
  Tprn.IndexName:='ukaz1';
  Tprn.Open;
...
var bank:string;
    codb:integer;
begin
 Lbitogo.Caption:=FloatToStrF(StrToFloat(Trim(MEo.Text)),ffFixed,9,2);
 if Tprn.Locate('delo',Edelo.Text,[])
     then
      begin
       ShowMessage('Этому пенсионеру доплата уже подсчитана!');
...
begin
 //if not Tprn.Eof then
  //begin
   ...
   Con.WriteStrData('',FloatToStrF(Tprn['dopl'],ffFixed,9,2));
   Tprn.Next;
   nn:=nn+1;
  //end;
